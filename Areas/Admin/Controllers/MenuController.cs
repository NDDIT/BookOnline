...
namespace NguyenDuyDuong.SachOnline.Areas.Admin.Controllers
{
    public class MenuController : Controller
    {
        ...
        public ActionResult Index()
            ...
            [HttpPost]
            public JsonResult Update(int id, string strTenMenu, string strLink, byte STT)
            {
                try
                {
            ...
            public ActionResult AddMenu(FormCollection f)
            {
                if (!String.IsNullOrEmpty(f["ThemChude"]))
                {
                    ...
                    m.OrderNumber = byte.Parse(f["Number"]);
                ...
                else if (!String.IsNullOrEmpty(f["ThemTrang"]))
                {
                    ...
                    m.OrderNumber = byte.Parse(f["Number1"]);
                    db.MENUs.InsertOnSubmit(m);
                    db.SubmitChanges();
                }
                else if (!String.IsNullOrEmpty(f["ThemLink"]))
                {
                    ...
                    m.OrderNumber = byte.Parse(f["Number2"]);
                    db.MENUs.InsertOnSubmit(m);
                    db.SubmitChanges();
                }
